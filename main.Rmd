...
home <- data$games.csv %>%
  filter(league_id == act_league, as.Date(date) <= as.Date(curr_date), as.Date(date) >= as.Date(opening_day), game_type == 0, if(league == 'NBC'){as.Date(date) != '2022-07-25'}) %>%
  mutate(
    home_id = home_team,
    runs_home = runs1,
    hits_home = hits1,
...
home_standings <- games %>%
  ...
  select(team, wins, losses, runs_scored, runs_allowed, run_diff, hits, hits_allowed, errors, errors_allowed, div_wins, div_loss, conf_wins, conf_loss, interleague_win, interleague_loss, one_run_win, one_run_loss, two_run_win, two_run_loss, three_run_win, three_run_loss, four_run_win, four_run_loss) %>%
  arrange(team)
